# Defecation Reflex + Hirschsprung Disease
> ### Headings
> ![alt text](images/HeadingsDefacn.png)

## 1. Defecation Reflex

> **Defecation = reflex phenomenon under voluntary control**

### **Stimulus**

$$
\boxed{\text{Mass peristalsis of descending + sigmoid colon}}
\rightarrow
\text{Rectal filling}
\rightarrow
\text{Stretch of rectal wall}
$$

### **Reflex pathway**

```mermaid
flowchart TD
    A["Rectal filling"] --> B["Stretch receptors in rectal wall"]
    B --> C["Pelvic nerve - afferent"]
    C --> D["Sacral spinal cord S3"]
    D --> E["Pelvic nerve - efferent"]
    D --> F["Somatic nerve - external anal sphincter"]
    E --> G["Rectum + Internal anal sphincter"]
    F --> H["External anal sphincter"]
    G --> I["Relaxation"]
    H --> J["Voluntary relaxation"]
    I --> K["Defecation"]
    J --> K
```

> ![alt text](images/DefecationRflxPathway.png)

### **Mechanism**

| Event                                    | Effect                              |
| ---------------------------------------- | ----------------------------------- |
| Rectal distension                        | Initiates defecation reflex         |
| Myenteric inhibitory signals             | **Relax internal anal sphincter**   |
| Voluntary cortical control               | **Relaxes external anal sphincter** |
| Puborectalis relaxation                  | **Decreases anorectal angle**       |
| Deep inspiration + diaphragm contraction | ↑ Intra-abdominal pressure          |
| Abdominal muscle contraction             | Further ↑ intra-abdominal pressure  |
| Straining                                | Forces feces through anal canal     |

$$
\boxed{\text{↑ Intra-abdominal pressure} ;+; \text{Sphincter relaxation}
\rightarrow \text{Defecation}}
$$

**Normal anorectal angle:** ~90°

**Applied:** Defecation is fundamentally a **spinal reflex**; higher centres, especially the cortex, exert voluntary control.

---

# 2. Hirschsprung Disease

### **Definition**

**Hirschsprung disease = congenital megacolon due to aganglionosis.**

### **Pathophysiology**

```mermaid
flowchart TD
    A["Congenital absence of ganglion cells"] --> B["Myenteric + submucosal plexuses absent"]
    B --> C["Distal colon + rectum cannot relax"]
    C --> D["Internal anal sphincter fails to relax"]
    D --> E["Functional obstruction"]
    E --> F["Feces accumulate proximally"]
    F --> G["Progressive colonic distension"]
    G --> H["Congenital megacolon"]
```

### **Clinical Features**

* **Abdominal distension**
* **Anorexia**
* **Lassitude**
* Severe cases → symptoms may appear in the **newborn period**

### **Treatment**

→ **Surgical treatment** to remove/bypass the aganglionic segment and restore bowel passage.

---

## ⭐ Last-minute recall

$$
\boxed{
\text{Rectal filling}
\rightarrow
\text{Stretch receptors}
\rightarrow
S_3
\rightarrow
\text{Pelvic nerve}
\rightarrow
\text{Internal sphincter relaxation}
}
$$

$$
\boxed{
\begin{matrix}
\text{Hirschsprung}\\\
\text{No ganglion cells}
\rightarrow
\text{No relaxation}
\rightarrow
\text{Obstruction}
\rightarrow
\text{Megacolon}
\end{matrix}
}
$$
