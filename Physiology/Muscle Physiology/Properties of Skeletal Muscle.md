...
> ![alt text](images/StvsDurnCurve.png)

```mermaid
flowchart TD
    subgraph Strength_Duration_Parameters ["Characteristic Features of Curve"]
        R["<b>1. Rheobase (R)</b><br>Minimum strength of stimulus required to excite tissue."]
        UT["<b>2. Utilization Time (UT)</b><br>Minimum time required for rheobasic strength of stimulus (threshold strength)."]
        C["<b>3. Chronaxie (C)</b><br>Minimum time required for a stimulus with double the rheobasic strength."]
    end
...
```mermaid
flowchart TD
    Contract["Types of Muscle Contraction"]
    
    Contract --> IsoT["<b>Isotonic Contraction</b><br><i>(Iso = Same, Tonic = Tension)</i><br>• Tension remains SAME<br>• Length is ALTERED<br>• <i>Example:</i> Simple flexion of arm (shortening of muscle fibers but tension unchanged)."]
    
    Contract --> IsoM["<b>Isometric Contraction</b><br><i>(Iso = Same, Meter = Length)</i><br>• Length remains SAME<br>• Tension is INCREASED<br>• <i>Example:</i> Pulling a heavy object (muscles become stiff & increased tension, length unchanged)."]
...
```mermaid
flowchart TD
    MT["Maintenance of Muscle Tone"]
    
    MT --> SM["<b>In Skeletal Muscle</b><br>• <i>Neurogenic:</i> Due to continuous discharge of impulses from gamma motor neurons in anterior gray horn of spinal cord."]
    MT --> CM["<b>In Cardiac Muscle</b><br>• <i>Purely Myogenic:</i> Muscles themselves control tone."]
    MT --> SmM["<b>In Smooth Muscle</b><br>• <i>Myogenic:</i> Depends upon Ca²⁺ level & number of cross-bridges."]
