...
```mermaid
flowchart TD
    subgraph Resting_State ["Resting Muscle"]
        R1["Z-lines at normal distance"]
        R2["H-zone and I-bands clearly visible"]
        R3["A-band length constant"]
    end

    subgraph Contraction_State ["Contracted Muscle"]
        C1["Z-lines pulled closer together"]
        C2["H-zone disappears / narrows"]
        C3["I-band width reduces"]
        C4["A-band length remains unchanged"]
    end

    Resting_State -->|"Myosin pulls Actin (Sliding)"| Contraction_State

```

> ![alt text](images/Contraction.png)
