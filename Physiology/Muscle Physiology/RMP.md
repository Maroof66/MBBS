...
```mermaid
flowchart TD
    IonicImbalance["Ionic Imbalance Across Membrane"]
    IonicImbalance --> Pump["<b>1. Sodium–Potassium Pump</b><br>Electrogenic pump moving 3 Na⁺ out and 2 K⁺ in."]
    IonicImbalance --> Perm["<b>2. Selective Permeability of CM</b><br>Regulated via specific transport/leak channels."]
...
```mermaid
flowchart LR
    subgraph Inside ["Inside Cell (Negative: -90 mV)"]
        K_in["High K⁺ Concentration<br>(~140 mEq/L)"]
        Prot["Fixed Anions<br>(Proteins⁻, PO₄³⁻, SO₄²⁻)"]
    end

    subgraph Pump_Action ["Na⁺-K⁺ Pump & Channels"]
        P["Na⁺-K⁺ Pump<br>(3 Na⁺ OUT / 2 K⁺ IN)"]
        K_leak["K⁺ Leak Channels<br>(Open → K⁺ Efflux)"]
        Na_leak["Na⁺ Leak Channels<br>(Mostly Closed → Minor Na⁺ Influx)"]
    end

    subgraph Outside ["Outside Cell (Positive)"]
        Na_out["High Na⁺ Concentration"]
        Cl_out["High Cl⁻ Concentration"]
    end

    K_in -->|Diffusion| K_leak --> Na_out
    Na_out -->|Slow Leak| Na_leak --> K_in
    K_in <==> P <==> Na_out
