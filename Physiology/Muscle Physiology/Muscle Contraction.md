...
```mermaid
flowchart TD
    A["Excitation of muscle fibers by impulses passing through motor nerve and neuromuscular junction"] --> B["Generation of action potential in muscle fiber"]
    B --> C["Spread of action potential via 'T' tubules"]
    C --> D["Entrance of action potential in cisternae"]
    D --> E["Release of calcium ions from cisternae"]
    E --> F["Movement of calcium ions towards actin filaments"]
    F --> G["Contraction of the muscle fibers"]

```

> ![alt text](images/ACcoupling.png)
...
```mermaid
flowchart TD
    A["Ca²⁺ ions released from 'L' tubules during excitation of muscle"] --> B["Bind with Troponin C"]
    B --> C["Loading of Troponin C with Ca²⁺ ions produces change in position of troponin molecules"]
    C --> D["Pulls tropomyosin molecule away from F-actin"]
    D --> E["Active site of F-actin uncovered & exposed"]
    E --> F["Immediately, head of myosin gets attached to actin"]
...
```mermaid
flowchart TD
    S1["Release of large amount of calcium ions from cisternae of 'L' tubules"] --> S2["Binding of calcium ions with troponin"]
    S2 --> S3["Change in position of troponin"]
    S3 --> S4["Pulling of tropomyosin molecule away from F actin"]
    S4 --> S5["Exposure of active sites of F actin"]
    S5 --> S6["Attachment of myosin head to actin"]
    S6 --> S7["Power stroke – tilting of myosin head and dragging of actin filament"]
    S7 --> S8["Detachment of myosin head and returning to original position"]
    S8 --> S9["Attachment of myosin head to new active site of F actin and dragging of actin filament – repetition of these actions"]
    S9 --> S10["Movement of actin filaments of both sides towards each other and formation of actomyosin complex"]
    S10 --> S11["Contraction of muscle"]

```

> ![alt text](images/changesdurncontrnofmuscle.png)
...
> ![alt text](images/slidingMech.png)
...
```mermaid
flowchart TD
    R1["Pumping of calcium ions into 'L' tubules"] --> R2["Decrease in sarcoplasmic calcium content"]
    R2 --> R3["Release of calcium ions from troponin C"]
    R3 --> R4["Detachment of myosin head from F actin"]
    R4 --> R5["Muscular relaxation"]

```

> ![alt text](images/muscleRelaxn.png)