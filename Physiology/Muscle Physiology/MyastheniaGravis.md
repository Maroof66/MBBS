...
```mermaid
flowchart TD
    Drug["Neostigmine & Pyridostigmine<br>(Cholinesterase Inhibitors)"] --> Inhibit["Inhibit acetylcholinesterase<br>(Enzyme that degrades ACh)"]
    Inhibit --> Accumulate["ACh remains in synaptic cleft for longer period"]
    Accumulate --> Bind["ACh can bind with available receptors<br>→ Improves neuromuscular transmission"]
```